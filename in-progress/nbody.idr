...
--
-- TODONOTE ported this to ghc7 first, see nbody.hs (not .ghc)
-- TODONOTE ghci nbody, then use :t <exp> to figure out types

-- TODONOTE *. in haskell is double -> vec3 -> vec3 (ie scale)?
-- TODONOTE in ghci use let Vec a b c = 2.0 *. Vec 1 2 3 etc to confirm behaviour

-- TODONOTE not sure if this is the right type to use
Double : Type
Double = Float
...
-- TODONOTE no bang patterns, !e !p
energy : Double -> Ptr Double -> IO Double
energy e p = case (p == end) of
    True  => return e
    False => let p2 = next p in do 
        p1 <- getVec (pos p)
        v1 <- getVec (vel p)
        m1 <- mass p
...
-- TODONOTE type signatures available from ghci
energy2 : Vector3 -> Double -> Double -> Ptr Double -> IO Double
energy2 p1 m1 e p = case p == end of
                         True  => return e
                         False => do
                            p2 <- getVec (pos p)
                            v2 <- getVec (vel p)
...
go : Ptr Double -> Vector3 -> Ptr Double -> Double -> Ptr Double -> IO ()
go p2 pos1 vel1 m1 p1 = case p2 /= end of
                             True  =>  do
                                          pos2 <- getVec (pos p2) 
                                          m2   <- mass p2
                                          let vel2       = vel p2
